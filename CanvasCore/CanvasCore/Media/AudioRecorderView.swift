    ...
    fileprivate enum State {
        ...
        fileprivate func recordButtonTapped(_ view: AudioRecorderView) {
            ...
            case .preRecording(AVAudioSession.RecordPermission.denied):
                ...
                let error = NSError(domain: "com.eskwelabs", code: 0, userInfo: [NSLocalizedDescriptionKey: message])
    ...
    @objc func finishRecordingWithRecorder(_ recorder: AudioRecorder) {
        ...
        do {
            if let fileURL = recorder.recordedFileURL {
                ...
                let error = NSError(domain: "com.eskwelabs", code: 0, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("The file does not exist.", tableName: "Localizable", bundle: .core, value: "", comment: "Error message for a file missing")])
                notifyUserOfError(error, title: recordingErrorTitle)
            }
        } catch let error as NSError {
            notifyUserOfError(error, title: recordingErrorTitle)

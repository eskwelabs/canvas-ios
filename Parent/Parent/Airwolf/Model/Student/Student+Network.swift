        ...
        return try getEnrollments(session: session)
            .flatMap(.concat, { (enrollments) -> SignalProducer<[JSONObject], NSError> in
                if (!enrollments.any(hasObserverEnrollment)) {
                    let error = NSError(domain: "com.eskwelabs.Enrollments", code: Error.NoObserverEnrollments, userInfo: [NSLocalizedDescriptionKey: "User has no observer enrollments"])
